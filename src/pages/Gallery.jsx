...
const Gallery = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-arabic pb-20" dir="rtl">
      ...
      {/* 🟢 الهيدر الجديد - ديزاين متناسق مع باقي الصفحات */}
      <div className="pt-16 pb-8 text-center bg-gray-50">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-[#7A0000] mb-4 relative inline-block">
          معرض الألبومات
          <span className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-16 h-1.5 bg-[#D4AF37] rounded-full"></span>
        </h1>
          ...
          {['الكل', 'مباراة', 'تدريب', 'حدث'].map(cat => (
            <button 
              ...
              className={`px-6 py-2.5 rounded-full font-bold transition-all shadow-sm ${
                filter === cat 
                ? 'bg-[#D4AF37] text-white transform scale-105 shadow-md' 
          ...
          {filteredAlbums.map((album) => (
            ...
            >
              <div className="h-56 relative bg-black">
                ...
                <div className="absolute top-3 right-3 bg-white/90 text-[#7A0000] text-xs font-black px-3 py-1.5 rounded-full shadow-md backdrop-blur-sm">
                  {album.category}
                </div>
              ...
              <div className="p-4 flex-1 flex flex-col justify-center">
                <h3 className="font-bold text-[#7A0000] text-lg text-center line-clamp-2">{album.title}</h3>
      ...
      {selectedAlbum && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex flex-col p-4 md:p-8 backdrop-blur-md" onClick={() => setSelectedAlbum(null)}>
          ...
          <div className="flex justify-between items-center text-white mb-6 w-full max-w-6xl mx-auto" onClick={e => e.stopPropagation()}>
            <div>
              <h2 className="text-2xl md:text-3xl font-black text-[#D4AF37] mb-1">{selectedAlbum.title}</h2>
